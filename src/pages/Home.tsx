    ...
    const fetchAvailableDates = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/api/available-dates?index=${indexName}`,
        );
        if (!res.ok) throw new Error('Failed to fetch available dates');
        const data = await res.json();
        const parsedDates = data.available_dates.map(
          (d: string) =>
            new Date(
              Date.UTC(
    ...
    const fetchLatestDate = async () => {
      try {
        ...
        if (!isNaN(parsed.getTime())) {
          ...
          ); // shift to 12:00 UTC to avoid selecting midnight, which is earlier than the first forecast time in NetCDF files
        ...
        >
          ...
          {isDateInAvailableDates(selectedDate, availableDates) ? (
            ...
            <div className="forecast-error">
              ⚠️ No data available for {selectedDate.toDateString()} in index "
              {indexName}". Please choose a different date.
            </div>
          )}
